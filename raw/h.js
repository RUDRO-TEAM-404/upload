...
module.exports = {
  ...
  async xyz({ bot, chat, msg, chatId, addListener, config }) {
    ...
    const downloadCondition = (incomingMsg) => {
      ...
      if (textContent) {
        return meta.keyword.some(urlPrefix =>
          ...
          (textContent.startsWith(urlPrefix) && !textContent.substring(urlPrefix.length).includes(' '))
        );
      }
    ...
    addListener(
      ...
      async (incomingMsg) => {
        ...
        try {
          ...
          } else if (messageText.includes("youtube") || messageText.includes("youtu.be") || messageText.includes("youtu.be")) {
            platform = "youtube";
